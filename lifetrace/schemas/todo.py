...
说明：
- 该模块面向 lifetrace-frontend 的 Todo 结构（支持 deadline/priority/tags/attachments 等）
- 数据库存储使用 lifetrace.storage.models 中的 Todo/Tag/Attachment 相关模型
"""
...
class TodoReorderItem(BaseModel):
    ...
    id: int = Field(..., description="待办ID")
    order: int = Field(..., description="新的排序序号")
