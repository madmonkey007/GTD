 ...
 * Agno 模式工具选择器组件
 * 显示为一个按钮，点击后展开多选下拉框
 * 支持 GTD 工具和外部工具（如 DuckDuckGo 搜索）
 */
export function ToolSelector({ disabled = false, isOpen: externalIsOpen, onOpenChange }: ToolSelectorProps) {
	...
	const handleSelectAllGtd = () => {
		setSelectedAgnoTools(GTD_TOOLS.map((tool) => tool.id));
	};

	const handleDeselectAllGtd = () => {
	...
	// 计算选中数量
	const gtdSelectedCount = selectedAgnoTools.length;
	const externalSelectedCount = selectedExternalTools.length;
	const totalSelectedCount = gtdSelectedCount + externalSelectedCount;
	const totalToolsCount = GTD_TOOLS.length + EXTERNAL_TOOLS.length;
						...
						<div>
							<div className="flex items-center justify-between mb-2 px-2">
								<div className="flex items-center gap-1.5">
									...
									<span className="text-xs font-medium text-muted-foreground">
										{tChat("toolSelector.gtdTools")}
									</span>
								</div>
								<div className="flex gap-2">
									<button
										type="button"
										onClick={handleSelectAllGtd}
										className="text-xs text-primary hover:underline"
									>
										{tChat("toolSelector.selectAll")}
									</button>
										...
										{tools.map((tool) => {
											...
											return (
												<button
													key={tool.id}
													type="button"
													onClick={() => handleToggleGtdTool(tool.id)}
													className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-accent"
												>
													<div
														className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border ${
